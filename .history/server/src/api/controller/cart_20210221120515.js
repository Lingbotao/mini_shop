  ...
  async checkoutAction() {
    ...
    if (addressId) {
      checkedAddress = await this.model("address")
        .where({ is_default: 1, user_id: this.getLoginUserId() })
        .find();
    } else {
      checkedAddress = await this.model("address")
    ...
    if (!think.isEmpty(checkedAddress)) {
      checkedAddress.province_name = await this.model("region").getRegionName(
        checkedAddress.province_id
      );
      checkedAddress.city_name = await this.model("region").getRegionName(
        checkedAddress.city_id
      );
      checkedAddress.district_name = await this.model("region").getRegionName(
