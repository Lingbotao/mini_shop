...
module.exports = class extends Base {
    ...
    async deleteAction() {
        ...
        // 删除当天的同一个商品的足迹
        const goods = await this.model("footprint")
            .where({user_id: userId, id: footprintId})
            .find();
        await this.model("footprint")
            .where({user_id: userId, goods_id: goods.goods_id})
            .delete();
    ...
    async listAction() {
        ...
        list.data = _.map(
            ...
            (item) => {
                ...
                // 昨天
                if (
                    moment().subtract(1, "days").format("YYYY-MM-DD") === item.add_time
                ) {
                    item.add_time = "昨天";
                }
                // 前天
                if (
                    moment().subtract(2, "days").format("YYYY-MM-DD") === item.add_time
                ) {
                    item.add_time = "前天";
                }
                return item;
